...
cat >/usr/local/bin/install-ollama.sh <<'EOF_OLLAMA'
#!/bin/bash
set -euxo pipefail
...
if [ -z "$OLLAMA_HOME" ]; then
    echo "Unable to determine the ollama user's home directory"
    exit 1
fi

sudo -u ollama env HOME="$OLLAMA_HOME" ollama list >/dev/null

if ! sudo -u ollama env HOME="$OLLAMA_HOME" ollama list \
    | awk 'NR > 1 {print $1}' \
    | grep -qx 'all-minilm:l12-v2'; then
...
cat >/etc/systemd/system/install-ollama.service <<'EOF_OLLAMA_SERVICE'
[Unit]
